...
import { getSubmissionById } from '../../services/submissionApi';
              ...
              rows: [
                ...
                new TableRow({
                  children: [
                    ...
                    new TableCell({
                      children: [new Paragraph({ text: `${Math.round(info.executionMemories / 1024)}`, heading: HeadingLevel.HEADING_3 })],
                    }),
                  ],
                }),
              ],
                ...
                ...info.details.map((submission, index) => {
                  return new TableRow({
                    children: [
                      ...
                      new TableCell({
                        children: [new Paragraph({ text: `${Math.round(submission.executionMemories / 1024)}`, heading: HeadingLevel.HEADING_3 })],
                      }),
                    ],
                  })
                })
...
class Report extends Component {
  ...
  generate = async () => {
    ...
    getSubmissionById(id).then((res) => {
      // console.log(res);
      const doc = documentCreator.create(res.data);
  ...
  render = () => {
    return (
      <div>
        <p>
          <Button size={this.props?.title ? '' : 'small'} onClick={this.generate}>{this.props?.title ? this.props?.title : 'Download'}</Button>
        </p>
      </div>
    );
  }
